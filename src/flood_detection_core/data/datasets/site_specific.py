...
class SiteSpecificTrainingDataset(Dataset):
    ...
    def __init__(
        ...
        # Pair sampling
        pair_sampling_strategy: Literal["all_pairs", "random"] = "all_pairs",
        num_pairs_per_epoch: int | None = None,
        # Patch sampling within a pair
        patch_sampling_strategy: Literal["random", "grid"] = "grid",
        patches_per_pair: int = 1,
        random_offset_per_epoch: bool = True,
        ...
        self.random_offset_per_epoch = random_offset_per_epoch
        self._epoch = 0
        # Seed for deterministic per-epoch sampling if provided
        ...
        self.site_tile_to_paths = self._load_pre_flood_split_csv(
            self.pre_flood_split_csv_path, self.dataset_type, set(self.sites)
        )

        # Build list of tile pairs per site
        self.site_tile_pairs: list[tuple[str, str, str]] = []  # (site, tileA, tileB)
        for site, tiles_dict in self.site_tile_to_paths.items():
            ...
            for a, b in itertools.combinations(sorted(tiles), 2):
                self.site_tile_pairs.append((site, a, b))

        if self.pair_sampling_strategy == "all_pairs":
            # Repeat each pair patches_per_pair times per epoch
            self._dataset_length = len(self.site_tile_pairs) * self.patches_per_pair
        else:
            if self.num_pairs_per_epoch is None:
                raise ValueError("num_pairs_per_epoch must be set when pair_sampling_strategy='random'")
            self._dataset_length = self.num_pairs_per_epoch

        # Prepare augmentation configs for pairing rules
        self._non_geometric_only_config = self._build_non_geometric_only_config(self.augmentation_config)

        # Per-epoch state for grid-based sampling (no image IO here)
        # One entry per pair: dict with keys: seq_paths_a, seq_paths_b, min_h, min_w, coords (list[(i,j)])
        self._pair_epoch_state: list[dict] = []
        if self.pair_sampling_strategy == "all_pairs" and self.patch_sampling_strategy == "grid":
            self._prepare_epoch_state()

    @staticmethod
    def _load_pre_flood_split_csv(
        split_csv_path: Path, dataset_type: Literal["train", "val", "test", "pretrain"], sites_filter: set[str]
        ...
        with open(split_csv_path) as f:
            ...
            for row in reader:
                ...
                mapping[site][tile].append(row["path"])  # keep as relative path string
        return mapping
    ...
    def _sample_tile_pair(self, idx: int) -> tuple[str, str, str]:
        if self.pair_sampling_strategy == "all_pairs":
            if self.patch_sampling_strategy == "grid":
                # Map global idx to (pair_idx, rep_idx)
                pair_idx = idx // self.patches_per_pair
                return self.site_tile_pairs[pair_idx]
            return self.site_tile_pairs[idx]
        # random strategy
        return random.choice(self.site_tile_pairs)
    ...
    def _choose_valid_patch_coords(
        self, seq_paths_a: list[Path | str], seq_paths_b: list[Path | str]
    ) -> tuple[int, int, int, int]:
        # Determine min common H/W across both sequences
        min_h_a = min(get_image_size(Path(p)) for p in seq_paths_a)[0]
        min_w_a = min(get_image_size(Path(p)) for p in seq_paths_a)[1]
        min_h_b = min(get_image_size(Path(p)) for p in seq_paths_b)[0]
        min_w_b = min(get_image_size(Path(p)) for p in seq_paths_b)[1]

        min_h = min(min_h_a, min_h_b)
        min_w = min(min_w_a, min_w_b)

        if min_h < self.patch_size or min_w < self.patch_size:
            raise ValueError("Chosen sequences too small for requested patch size")

        max_i = min_h - self.patch_size
        max_j = min_w - self.patch_size
        i = random.randint(0, max_i)
        j = random.randint(0, max_j)
        return i, j, min_h, min_w

    # -------- Grid sampling helpers (no heavy IO here) --------
    def _rng_for_pair(self, pair_idx: int) -> random.Random:
        seed = (self._base_seed + 97 * self._epoch + 7919 * pair_idx) % (2**31 - 1)
        return random.Random(seed)

    def _prepare_epoch_state(self) -> None:
        """
        Prepare per-pair epoch state: chosen temporal sequences, common grid with optional random offset,
        and a randomized order of grid coordinates. Avoids reading image pixels here.
        """
        self._pair_epoch_state = []
        stride = self.patch_size
        for pair_idx, (site, tile_a, tile_b) in enumerate(self.site_tile_pairs):
            rng = self._rng_for_pair(pair_idx)
            # Choose a consecutive sequence of num_temporal_length per tile deterministically for this epoch
            seq_paths_a = self._pick_sequence_paths(site, tile_a)
            seq_paths_b = self._pick_sequence_paths(site, tile_b)

            # Compute common min height/width without loading arrays
            min_h_a = min(get_image_size(Path(p)) for p in seq_paths_a)[0]
            min_w_a = min(get_image_size(Path(p)) for p in seq_paths_a)[1]
            min_h_b = min(get_image_size(Path(p)) for p in seq_paths_b)[0]
            min_w_b = min(get_image_size(Path(p)) for p in seq_paths_b)[1]
            ...
            if min_h < self.patch_size or min_w < self.patch_size:
                # Skip unusable pair for this epoch; store empty coords and let __getitem__ resample
                self._pair_epoch_state.append(
                    {
                        "seq_paths_a": seq_paths_a,
                        "seq_paths_b": seq_paths_b,
                        "min_h": min_h,
                        "min_w": min_w,
                        "coords": [],
                    }
            ...
            # Random offset per epoch (optional)
            if self.random_offset_per_epoch:
                offset_i = rng.randrange(0, stride)
                offset_j = rng.randrange(0, stride)
            else:
                offset_i = 0
                offset_j = 0

            # Build non-overlapping grid with stride = patch_size
            i_positions = list(range(offset_i, min_h - self.patch_size + 1, stride))
            j_positions = list(range(offset_j, min_w - self.patch_size + 1, stride))
    ...
    def set_epoch(self, epoch: int) -> None:
        """Update epoch index and refresh per-epoch state for grid sampling."""
    ...
    def __getitem__(self, idx: int):
        for _ in range(20):
            site, tile_a, tile_b = self._sample_tile_pair(idx)
            try:
                if self.pair_sampling_strategy == "all_pairs" and self.patch_sampling_strategy == "grid":
                    pair_idx = idx // self.patches_per_pair
                    rep_idx = idx % self.patches_per_pair
                    state = self._pair_epoch_state[pair_idx]
                    seq_paths_a = state["seq_paths_a"]
                    seq_paths_b = state["seq_paths_b"]
                    coords = state["coords"]
                    if not coords:
                        i, j, _, _ = self._choose_valid_patch_coords(seq_paths_a, seq_paths_b)
                    else:
                        coord_idx = rep_idx % len(coords)
                        i, j = coords[coord_idx]
                else:
                    seq_paths_a = self._pick_sequence_paths(site, tile_a)
                    seq_paths_b = self._pick_sequence_paths(site, tile_b)
                    i, j, _, _ = self._choose_valid_patch_coords(seq_paths_a, seq_paths_b)
        ...
        raise RuntimeError("Failed to sample a valid site/tile pair after multiple attempts")
...
if __name__ == "__main__":
    ...
    dataset = SiteSpecificTrainingDataset(
        ...
        pair_sampling_strategy="all_pairs",
        patch_sampling_strategy="grid",
        patches_per_pair=4,
        num_pairs_per_epoch=8,
