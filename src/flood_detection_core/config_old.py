...
class UrbanSARConfig(DataDirConfig):
    no_flood_gt_dir: str | Path
    no_flood_sar_dir: str | Path
    flood_urban_gt_dir: str | Path
    flood_urban_sar_dir: str | Path
    flood_open_gt_dir: str | Path
    flood_open_sar_dir: str | Path


class UrbanSARNewConfig(DataDirConfig):
    sites_metadata: str | Path
    metadata_dir: str | Path
    nf_pre_flood_base_dir: str | Path
    nf_pre_flood_dir: str | Path
    nf_post_flood_dir: str | Path
    nf_gt_dir: str | Path
    fu_pre_flood_base_dir: str | Path
    fu_pre_flood_dir: str | Path
    fu_post_flood_dir: str | Path
    fu_gt_dir: str | Path
    fo_pre_flood_base_dir: str | Path
    fo_pre_flood_dir: str | Path
    fo_post_flood_dir: str | Path
    fo_gt_dir: str | Path
...
class DataConfig(BaseSettingsWithYaml):
    ...
    urban_sar: UrbanSARConfig
    urban_sar_new: UrbanSARNewConfig

    @model_validator(mode="before")
    def validate_paths(cls, values):
        if isinstance(values, dict):
            for config_name in ["gee", "hand_labeled_sen1flood11", "artifact", "splits", "urban_sar", "urban_sar_new"]:
